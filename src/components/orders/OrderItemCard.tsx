...
export default function OrderItemCard({ item, isStaff, ordererColorIndex }: OrderItemCardProps) {
  ...
  // Staff용 색상 바
  const colorBarClass = ordererColorIndex !== undefined
      ...
      <div className="flex">
        ...
        <div className="flex-1 min-w-0">
          ...
          {product.specifications && (
            <p className="text-xs text-gray-400 mb-1">{product.specifications}</p>
          )}
          ...
          {isStaff && (
            <div className="flex justify-end mt-1 pt-1 border-t border-gray-100">
              <span className="text-xs text-gray-500">
                주문 총액: <span className="font-semibold text-gray-700">{item.totalOrderPrice.toLocaleString()}원</span>
              </span>
            </div>
          )}
        </div>
